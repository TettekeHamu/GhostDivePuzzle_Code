...
{
    /// <summary>
    /// 扇風機オブジェクトにダイブ中 & 落下中のState　FanDiveFallingState
    /// </summary>
    public class FanDiveFallingState : IHamuState, ICollisionEnemy, IDisposable
    {
        /// <summary>
        ...
        /// コンストラクター
        /// </summary>
        public FanDiveFallingState(ITransitionState<PlayerStateType> ts, PlayerComponentController pcc)
        {
            transitionState = ts;
            playerComponent = pcc;
        ...
        private async UniTaskVoid AsyncStartFalling(CancellationToken token)
        {
            ...
            {
                //落下させる
                playerComponent.AnimationManager.PlayerAnimator.SetBool(playerComponent.AnimationManager.IsFanFalling, true);
